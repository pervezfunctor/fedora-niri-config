...
```bash
bash -c '$(curl -fsSL https://raw.githubusercontent.com/pervezfunctor/fedora-niri-nix-config/main/setup)'
```

The bootstrap script clones the repo to `~/.local/share/fedora-niri-nix-config`, installs Nushell if needed, and runs the fish setup.
